...
//So when dynamic_cast for a pointer type fails it returns a null pointer and the caller
//can check for that, but when it fails for a reference type it can't return a null reference,
